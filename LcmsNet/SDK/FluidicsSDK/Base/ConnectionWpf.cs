...
namespace FluidicsSDK.Base
{
    public sealed class ConnectionWpf : IRenderableWpf
    {
        ...
        public ConnectionWpf(PortWpf from, PortWpf to, FluidicsDeviceWpf device, ConnectionStyles? style = null)
        {
            //assign the availableId to this, and then increment it so the next class gets availableId+1 as its id.
            m_id = availableId++;
        ...
        /// <param name="to"></param>
        /// <returns></returns>
        private List<GraphicsPrimitiveWpf> SetupLines(PortWpf from, PortWpf to)
        {
            var lines = new List<GraphicsPrimitiveWpf>();
            if (ConnectionStyle == ConnectionStyles.Elbow)
            {
                ...
                var line = new FluidicsLineWpf(from.Center, midPoint);
                lines.Add(line);
                // elbow off to port "to"
                line = new FluidicsLineWpf(midPoint, to.Center);
                lines.Add(line);
            }
            else
            {
                var line = new FluidicsLineWpf(new Point(from.Center.X, from.Center.Y), new Point(to.Center.X, to.Center.Y));
                lines.Add(line);
            }
            return lines;
        }
        ...
        public void Render(DrawingContext g, byte alpha, float scale)
        {
            if (!Transparent)
            {
                ...
                {
                    var line = (FluidicsLineWpf)singleLine;
        ...
        public void MoveWith(PortWpf port)
        {
            if (ConnectionStyle == ConnectionStyles.Elbow)
            {
                ...
                var lineA = m_prims[0] as FluidicsLineWpf;
                if (lineA != null)
                {
                ...
                var lineB = m_prims[1] as FluidicsLineWpf;
                if (lineB != null)
                {
            ...
            {
                var line = m_prims[0] as FluidicsLineWpf;
                if (line != null)
                {
        ...
        public PortWpf FindOppositeEndOfConnection(PortWpf p)
        {
            if (p.ID == P1.ID)
            {
                return P2;
        ...
        /// </summary>
        public FluidicsDeviceWpf InternalConnectionOf
        {
            get
            {
                return m_device;
