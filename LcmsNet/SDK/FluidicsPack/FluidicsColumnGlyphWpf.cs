...
{
    public sealed class FluidicsColumnGlyphWpf : FluidicsDeviceWpf, IFluidicsDevice
    {
        ...
        public FluidicsColumnGlyphWpf()
        {
            var newsize = new Size(20, 200);

            GraphicsPrimitiveWpf primitive = new FluidicsRectangleWpf(new Point(0, 0), newsize, Colors.Black, Brushes.White);

            AddPrimitive(primitive);

            var points = GeneratePortLocs();
            var inputPort = new PortWpf(points[0], this)
            {
                Source = false,
                Sink = false
            };

            var exitPort = new PortWpf(points[1], this)
            {
                Source = false,
                Sink = false
            };
            ...
            AddPort(inputPort);
            AddPort(exitPort);
            ConnectionManagerWpf.GetConnectionManager.Connect(inputPort, exitPort, this);
            var c = ConnectionManagerWpf.GetConnectionManager.FindConnection(inputPort, exitPort);
            c.Transparent = true;
