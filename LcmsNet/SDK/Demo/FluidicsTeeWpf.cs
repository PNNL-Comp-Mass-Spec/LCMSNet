...
{
    public sealed class FluidicsTeeWpf : FluidicsDeviceWpf
    {
        readonly Dictionary<string, FluidicsLineWpf> m_primLines;
        public FluidicsTeeWpf()
        {
            m_primLines = new Dictionary<string, FluidicsLineWpf>();

            //left line going down
            var newLine = new FluidicsLineWpf(new Point(0, 0), new Point(0, 29));
            AddPrimitive(newLine);
            m_primLines["left"] = newLine;

            //left line angled in
            newLine = new FluidicsLineWpf(new Point(0, 28), new Point(10, 46));
            AddPrimitive(newLine);
            m_primLines["leftAngled"] = newLine;

            //bottom line across
            newLine = new FluidicsLineWpf(new Point(10, 46), new Point(58, 46));
            AddPrimitive(newLine);
            m_primLines["bottom"] = newLine;

            //right line angled in
            newLine = new FluidicsLineWpf(new Point(57, 46), new Point(70, 26));
            AddPrimitive(newLine);
            m_primLines["rightAngled"] = newLine;

            // right line going up
            newLine = new FluidicsLineWpf(new Point(70, 27), new Point(70, 0));
            AddPrimitive(newLine);
            m_primLines["right"] = newLine;

            //top line going from right line to left line
            newLine = new FluidicsLineWpf(new Point(70, 0), new Point(0, 0));
            AddPrimitive(newLine);
