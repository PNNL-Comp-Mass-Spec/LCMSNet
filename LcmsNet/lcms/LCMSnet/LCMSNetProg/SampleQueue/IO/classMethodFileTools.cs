...
namespace LcmsNet.SampleQueue.IO
{
	...
	{		
		...
		public static void WriteMethodFiles(classSampleData sample)
		{
			...
			{
				string msg = "WriteMethodFiles: Sample " + sample.DmsData.DatasetName + ", Method folder creation disabled";
			...
			if (!shouldCopyFolder)
			{
				string msg = "The method data was not copied to the server for: " + sample.DmsData.DatasetName + ".  the method folder copy is disabled";
				classApplicationLogger.LogMessage(0, msg);
				return;
			}
        ...
        private static string MakeLocalMethodFolder(classSampleData sample)
		{
			...
			string localFolder = Path.Combine(classLCMSSettings.GetParameter("ApplicationPath"), LOCAL_METHOD_FOLDER_NAME);
        ...
        public static bool CheckLocalMethodFolders()
		{
			...
			string localMethodXferFolder = Path.Combine(classLCMSSettings.GetParameter("ApplicationPath"), LOCAL_METHOD_FOLDER_NAME);
			if (!Directory.Exists(localMethodXferFolder)) return false;	// If no directory, there are no folders needing transfer
        ...
        public static void MoveLocalMethodFiles()
		{
            ...
            {				
				try
				{
                    ...
                    classApplicationLogger.LogMessage(1, string.Format("Copying method file data.  Copied {0} to {1}.", file, remotePath));                                                 					                   					
				}
				catch (Exception ex)
				{					
