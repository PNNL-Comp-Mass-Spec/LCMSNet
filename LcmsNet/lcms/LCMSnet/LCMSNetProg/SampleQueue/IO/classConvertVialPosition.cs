	...
	class classConvertVialPosition
	{
		...
		#region "Methods"
			...
			public static string ConvertVialToString(int vialPosition)
			{
				...
				if ((vialPosition < 1) | (vialPosition > 96)) { return "Z99"; }
			...
			public static int ConvertVialToInt(string vialPosition)
			{
				...
				// Ensure valid string
				if (vialPosition.Length < 2) { return 99; }
				if (vialPosition.Length > 3) { return 99; }

				string tmpPosition = vialPosition.ToUpper();

				// Get first character
				mc = Regex.Matches(tmpPosition,"^[A-E]");
				if (mc.Count != 1) { return 99; }
				...
				if ((numericPart < 1) | (numericPart > 12)) { return 99; }
