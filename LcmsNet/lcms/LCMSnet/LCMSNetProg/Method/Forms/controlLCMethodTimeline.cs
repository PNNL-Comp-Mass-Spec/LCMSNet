    ...
    {        
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {               
                ...
                //down-pointing arrow
                arrowPoints[0] = new Point(downButton.X + (downButton.Width / 2), downButton.Y + 2*(downButton.Height/3));
                arrowPoints[1] = new Point(downButton.X + (downButton.Width / 4), downButton.Y +  (downButton.Height / 3));
                arrowPoints[2] = new Point(downButton.X + 3 * (downButton.Width / 4), downButton.Y + (downButton.Height / 3));
                e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
                e.Graphics.FillPolygon(Brushes.Black, arrowPoints);                       
            }
        }         
        ...
        private void controlLCMethodTimeline_MouseUp(object sender, MouseEventArgs e)
        {
            if(e.Button == System.Windows.Forms.MouseButtons.Left && RenderMode == enumLCMethodRenderMode.Conversation)
            {
                ...
                else if(buttonLocations[1].Contains(e.Location))
                {
                    ...
                    {
                        var maxIndex = FluidicsSimulator.FluidicsSimulator.BuildEventList(mlist_methods, mlist_methods[0].Start).Count - 1;
        ...
        private void controlLCMethodTimeline_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if(leftMouseDown && RenderMode == enumLCMethodRenderMode.Conversation)
            {
                ...
                if (mlist_methods.Count > 0)
                {
                    var maxIndex = FluidicsSimulator.FluidicsSimulator.BuildEventList(mlist_methods, mlist_methods[0].Start).Count - 1; //numer of lists of events that happen at the same time. This is the upper bound of the index we are trying to change and track.
