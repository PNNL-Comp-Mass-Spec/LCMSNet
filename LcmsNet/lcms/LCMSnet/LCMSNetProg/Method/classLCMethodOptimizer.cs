...
namespace LcmsNet.Method
{
    ...
    public class classLCMethodOptimizer
    {
        ...
        /// <summary>
        /// Time required to space the lc methods in seconds.
        /// </summary>
        private const int CONST_REQUIRED_LC_METHOD_SPACING_SECONDS = 500;
        /// <summary>
       ...
       // private int mint_numberOfColumns;
        ...
        public bool AlignMethods(List<classLCMethod> baselineMethods, classLCMethod aligneeMethod)
        {
            ...
            {
                aligneeMethod.SetStartTime(baselineMethods[k].End.AddMilliseconds(CONST_REQUIRED_LC_METHOD_SPACING_SECONDS));
                return true;
            }
            ...
            for (j = k - 1; j >= 0; j--)
            {
                ...
                if (colI == colJ && j > lastColumnContention)
                {
                    if (DateTime.Compare(aligneeMethod.Start, baselineMethods[j].End) < 0)
                    {
                        ...
                        aligneeMethod.SetStartTime(baselineMethods[j].End.AddMilliseconds(CONST_REQUIRED_LC_METHOD_SPACING_SECONDS));
                    }
                }
        ...
        public bool AlignMethods(List<classLCMethod> methods)
        {
            ...
            for (int i = 1; i < methods.Count; i++)
            {
                ...
                if (!methods[i].AllowPreOverlap || !methods[i-1].AllowPostOverlap)
                {
                    ...
                    if (colI == colJ && j > lastColumnContention)
                    {
                        startOfPrevious = methods[i - 1].End.AddMilliseconds(CONST_REQUIRED_LC_METHOD_SPACING_SECONDS);
                    }
                    else
                    {
                ...
                for (j = i - 1; j >= 0; j--)
                {
                    ...
                    if (colI == colJ && j > lastColumnContention)
                    {
                        ...
                        {
                            lastColumnContention = j;
                            methods[i].SetStartTime(methods[j].End.AddMilliseconds(CONST_REQUIRED_LC_METHOD_SPACING_SECONDS));
                        }
                    }
                    // We cannot just break here when there is no overlap,
                    // because even if method j starts after method j - n, method j-n may end
