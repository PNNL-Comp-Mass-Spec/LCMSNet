...
namespace LcmsNet.Method
{
    ...
    public class classLCMethodReader
    {
        ...
        private classLCEvent ReadEventNode(XmlNode node)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("Null Reference Exception due to backwards compatibility check in LCMethodReader");
            ...
            for (int i = 0; i < parameterList.Count; i++)
            {
                ...
                catch (FileNotFoundException fileEx)
                {
                    try
                    {
                        ...
                        {
                            throw new Exception(string.Format("Could not construct parameter for {0}.", parameterName.Value), fileEx);
                        }
                    }
                    catch (Exception newException)
                    {
                        constructedParameterType = null;
                        throw new Exception(string.Format("Could not construct parameter for {0}.", parameterName.Value), newException);
                    }
                }
        ...
        private classLCMethod ReadEventList(XmlNode root, bool readActual, ref List<Exception> errors)
        {
            ...
            foreach (XmlNode node in eventListNode)
            {
                ...
                {
                    string error  = string.Format("The Device \"{0}\" was missing from the hardware manager at event {1}.",
                                                  ex.DeviceName,
                                                  i);
