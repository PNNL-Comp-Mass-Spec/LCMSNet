...
namespace LcmsNet.Method.Drawing
{
    ...
    public class classLCMethodRenderer
    {
                        ...
                        foreach (classLCMethod method in methods)
                        {
                            foreach (classLCEvent lcEvent in method.Events)
                            {
                                if (lcEvent.OptimizeWith)
                                {
                                    ...
                                    float x                     = bounds.X + Convert.ToSingle(lcEvent.Start.Subtract(start).TotalSeconds) * ppt;
                                            ...
                                            Color                    backColor)
        {
            ...
            SolidBrush outlineBrush = lcEvent.MethodData.Executing ? new SolidBrush(Color.Green) : new SolidBrush(Color.Black);
                                            ...
                                            DateTime progress)
        {
            ...
            if (methods != null && methods.Count > 0)
            {
                ...
                /// 
                float pixelsPerSecond = Convert.ToSingle(bounds.Width - bounds.X) / Convert.ToSingle(methodDuration.TotalSeconds);
