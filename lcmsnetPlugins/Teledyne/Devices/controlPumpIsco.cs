...
namespace LcmsNet.Devices.Pumps
{
    public partial class controlPumpIsco : controlBaseDeviceControl, IDeviceControl
    {
        ...
        private void InitControl()
        {
            ...
            {
                mcomboBox_Ports.Items.Add(portName);
            }

            if (mcomboBox_Ports.Items.Count > 0)
                mcomboBox_Ports.SelectedIndex = 0;
            ...
            if (itemIndx != -1)
            {
                ...
                mcomboBox_UnitAddress.SelectedIndex = 0;
        ...
        private void UpdateLimitDisplay()
        {
            ...
            for (var indx = 0; indx < m_PumpDisplays.Count; indx++)
            {
                ...
                // Min press SP
                itemArray[3].SubItems.Add(m_PumpDisplays[indx].MinPressSp.ToString("0.000"));

                // Max press SP
                itemArray[4].SubItems.Add(m_PumpDisplays[indx].MaxPressSp.ToString("0.000"));

                // Min flow SP
                itemArray[5].SubItems.Add(m_PumpDisplays[indx].MinFlowSp.ToString("0.000"));

                // Max flow SP
                itemArray[6].SubItems.Add(m_PumpDisplays[indx].MaxFlowSp.ToString("0.000"));

                // Max flow limit
                itemArray[7].SubItems.Add(m_PumpDisplays[indx].MaxFlowLimit.ToString("0.000"));
        ...
        void m_PumpDisplays_SetpointChanged(object sender, int pumpIndex, double newValue)
        {
            ...
            if (m_Pump.OperationMode == enumIscoOperationMode.ConstantFlow)
            {
                ...
                success = m_Pump.SetPressure(pumpIndex, newValue);
        ...
        private void mbutton_SetAllFlow_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!m_Pump.SetFlow(indx, m_PumpDisplays[indx].Setpoint))
                    success = false;
            }
        ...
        private void mbuttonSetAllPress_Click(object sender, EventArgs e)
        {
            ...
            for (var indx = 0; indx < m_PumpCount; indx++)
            {
                if (!m_Pump.SetPressure(indx, m_PumpDisplays[indx].Setpoint))
                    success = false;
            }
        ...
        private void mbutton_SetRefillRate_Click(object sender, EventArgs e)
        {
            ...
            for (var indx = 0; indx < m_refillRates.Length; indx++)
            {
                ...
                {
                    UpdateStatusDisplay("New setpoint " + newSp.ToString("0.000") + " out of range");
                    return;
                }
            }
        }
        ...
        void m_Pump_ControlModeSet(enumIscoControlMode newMode)
        {
            if (newMode == enumIscoControlMode.Local)
            {
                ...
                mcomboBox_ControlMode.SelectedIndex = 1;
        }
